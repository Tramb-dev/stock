	...
	<div id="outer">
		...
		<div id="wrapper">
			<table id="etat_ref" class="tablesorter">
				<caption>Etat du stock de la cave</caption>
				<thead>
					<tr>
						<th colspan="2" class="type group-false">Type</th>
						<th class="stock group-false">Stock</th>
						<th class="next_dlc group-false">Prochaine DLC/DLUO</th>
						<th class="code group-false" colspan="2">Code</th>
					</tr>
				</thead>
				<tbody>
					<?php
					...
					$sqlCat = 'SELECT t.id, r.dlc_dluo, t.seuil, t.type, v.stock, v.expire 
									FROM Reference r 
									LEFT JOIN Type t ON t.id = r.t_id 
									LEFT JOIN V_Ref_grp_t_id v ON v.t_id = t.id 
									GROUP BY t.id
									ORDER BY t.type';
					$requete = $db->query($sqlCat);
					
					while($tab = $requete->fetch(PDO::FETCH_ASSOC)){
						if($tab['expire'] != null && compareDlc($tab['expire']) == true)
							$aConsommerCat = ' aConsommer';
						else
							$aConsommerCat = '';
						echo '<tr class="cat tablesorter-hasChildRow" id="c_' . $tab['id'] . '">';
						echo '<td class="type" colspan="2"><a href="#" class="toggle">' . $tab['type'] . '</a></td>';
						echo '<td class="stock">' . $tab['stock'] . '</td>';
						switch($tab['dlc_dluo']){
							case 0: 
								echo '<td class="dlc' . $aConsommerCat . '">' . $tab['expire'] . '<span>dlc</span></td>';
								break;
							
							case 1: 
								echo '<td class="dluo' . $aConsommerCat . '">' . $tab['expire'] . '<span>dluo</span></td>';
								break;
						...
						$sqlRef = 'SELECT r.id, r.marque, r.produit, r.quantite, r.unite, r.dlc_dluo, r.code, t.type, v.stock, v.r_id, v.expire 
										FROM Reference r 
										LEFT JOIN Type t ON t.id = r.t_id 
										LEFT JOIN V_Dlc_grp_r_id v ON v.r_id = r.id
										WHERE r.t_id = ' . $tab['id'] . ' AND r.lieu = 0';
						$req = $db->query($sqlRef);
						
 						while($ref = $req->fetch(PDO::FETCH_ASSOC)){
							if($ref['expire'] != null && compareDlc($ref['expire']) == true)
							...
							if(isset($quantite[1])){
								if($quantite[1] == '00')
									$quantiteAffich = $quantite[0] . ' ' . $convers[1];
								else
									$quantiteAffich = $quantite[0] . '.' . $quantite[1] . ' ' . $convers[1];
							}

							echo '<tr class="produit tablesorter-childRow" id="' . $ref['id'] . '">';
							echo '<td class="ajout"><span>+</span></td>';
							echo '<td class="marque">' . $ref['marque'] . ' ' . $ref['produit'] . '</td>';
							
							
							echo '<td class="stock">' . $ref['stock'] . ' * ' . $quantiteAffich . '</td>';
							switch($ref['dlc_dluo']){
								case 0: 
									echo '<td class="dlc' . $aConsommer . '">' . $ref['expire'] . '<span>dlc</span></td>';
									break;
								
								case 1: 
									echo '<td class="dluo' . $aConsommer . '">' . $ref['expire'] . '<span>dluo</span></td>';
									break;
									
								case 2: 
									echo '<td class="wt-dlc">' . $ref['expire'] . '</td>';
									break;
									
								default:
									break;
							}
							echo '<td class="barcode">' . $ref['code'] . '</td>';
